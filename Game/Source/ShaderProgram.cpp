...
namespace Blz
{
	namespace Graphics
	{
		...
		auto ShaderProgram::Init(std::string vertexShaderFilePath, std::string fragmentShaderFilePath) -> void
		{
		...
		{
			auto ReadShaderSourceFile = [](const char* c_ShaderFilePath, const char* c_TypeOfShader) -> std::string
			{
				...
				return std::string(std::istreambuf_iterator<char>(shaderFileInputStream), std::istreambuf_iterator<char>());
			};
			...
			std::string temp = ReadShaderSourceFile(vertexShaderFile.c_str(), "Vertex").c_str();
		...
		auto ShaderProgram::AddAttribute(std::string attributeNameFromShaderFile) -> void
		{
			glBindAttribLocation(programID, numAttributes++, attributeNameFromShaderFile.c_str());
		}

		auto ShaderProgram::GetUniformLocation(std::string uniformNameFromShaderFile) -> GLuint
		{
			GLuint location = glGetUniformLocation(programID, uniformNameFromShaderFile.c_str());
			return location;
		}
